...
impl Shape for Sphere {
    fn intersect_without_normal(&self, (ray_dir, ray_origin): Ray) 
            -> Option<f64> {
		let c = ray_origin.dot(ray_origin) + self.centre.dot(self.centre) - 2.0*ray_origin.dot(self.centre) - self.radius*self.radius;
    ...
    fn intersect(&self, (dir, origin): Ray) -> Option<(f64, Vector3)> {
        self.intersect_without_normal((dir, origin)).map(
            |t: f64| (t, (origin + dir * t) - self.centre)
        )
    }
}
